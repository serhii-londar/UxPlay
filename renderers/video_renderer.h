...
/* Multi-client mirroring: each "slot" is an independent appsrc-fed H264 or H265
 * pipeline, built from the -vrtp or -vrtp265 template (matching the negotiated
 * codec) as the singleton path above, bound to its own loopback port. Slots
 * don't touch the singleton `renderer` state at all, so this can run alongside
